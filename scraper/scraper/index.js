...
// ── SCRAPE ONE KEYWORD ──
async function scrapeKeyword(page, keyword, existingUrls, isFirstKeyword) {
  const results = [];
  log('SEARCH', `Searching for: ${keyword}`);

  try {
    if (isFirstKeyword) {
      // First keyword of the run — land on the homepage like a real visitor,
      // then use the search box to navigate. This avoids jumping straight to a
      // search URL, which TikTok may flag as more bot-like than organic browsing.
      log('NAVIGATE', 'Visiting TikTok homepage first (like a real visitor)...');
      await page.goto('https://www.tiktok.com/', { waitUntil: 'domcontentloaded', timeout: 30000 });
      await randomDelay(config.delays.pageLoad);

      try {
        // TikTok's search box on the homepage/nav bar
        const searchBoxSelectors = ['input[type="search"]', 'input[placeholder*="Search"]', 'input[data-e2e="search-user-input"]'];
        let searchBox = null;
        for (const sel of searchBoxSelectors) {
          searchBox = await page.$(sel);
          if (searchBox) { log('NAVIGATE', `Found search box with selector: ${sel}`); break; }
        }
        ...
        if (searchBox) {
          ...
          await searchBox.type(keyword, { delay: 80 + Math.random() * 60 });
          await randomDelay([800, 1500]);
          await page.keyboard.press('Enter');
          await randomDelay(config.delays.afterSearch);
          log('NAVIGATE', 'Submitted search via search box');
        } else {
          log('NAVIGATE', 'Could not find search box — falling back to direct URL');
          const searchUrl = `https://www.tiktok.com/search?q=${encodeURIComponent(keyword)}`;
          await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
        }
      } catch (e) {
        logError('NAVIGATE', 'Search box interaction failed — falling back to direct URL', e);
        const searchUrl = `https://www.tiktok.com/search?q=${encodeURIComponent(keyword)}`;
        await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
      }
    } else {
      // For subsequent keywords in the same run, we're already "browsing" —
      // going direct to the search URL here is fine since the session already looks organic.
      const searchUrl = `https://www.tiktok.com/search?q=${encodeURIComponent(keyword)}`;
    ...
    log('NAVIGATE', `Current URL: ${page.url()}`);
...
async function main() {
  ...
  // Retry Sheets connection up to 3 times — the OAuth token fetch can occasionally
  // hit a transient "Premature close" network error on the runner, unrelated to credentials
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      sheets = getSheetsClient();
      await ensureHeaders(sheets);
      existingUrls = await loadExistingLinks(sheets);
      log('SHEETS', `Connected successfully on attempt ${attempt}`);
      break;
    } catch (err) {
      logError('SHEETS', `Connection attempt ${attempt}/3 failed`, err);
      sheets = null;
      if (attempt < 3) {
  ...
  try {
    ...
    for (let i = 0; i < config.keywords.length; i++) {
      ...
      try {
        const results = await scrapeKeyword(page, keyword, existingUrls, i === 0);
